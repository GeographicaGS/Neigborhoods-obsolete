...
IndicatorModel.prototype.getIndicatorData = function(table,callback){
	BaseModel.query(callback,'select data.nombrebloq, n.nom_barr,n.city, n.town, data.valor, ST_X(ST_Centroid(ST_Transform(n.geom,4326))) as lng, ST_Y(ST_Centroid(ST_Transform(n.geom,4326))) as lat from data.' + table + ' data inner join data.neighborhoods n on data.nombrebloq = n.nombrebloq order by n.city');
};
